...
{/* Floating Price Popup - Desktop only */}
<div className="hidden lg:flex fixed bottom-6 right-6 z-50 bg-white shadow-xl border border-black/10 rounded-xl px-6 py-4 flex-col items-center gap-3 max-w-[300px]">
  <div className="text-2xl font-bold text-black">${selectedFeature?.price ?? "39,000"}</div>
  <div className="text-sm text-neutral-600">Est. Lease: ${selectedFeature?.lease ?? "359"}/mo</div>

  <button
    className="w-full relative overflow-hidden bg-[#08175e] text-white font-bold py-3 px-8 rounded-full hover:bg-black transition-colors duration-300 text-center"
    style={{ animation: "pulseZoom 2s infinite" }}
  >
    <span className="relative z-10">Configure Now</span>
...
<div className="lg:hidden w-full min-h-screen flex flex-col justify-between px-0 pt-6 pb-4 overflow-hidden">
  ...
  <div className="flex items-center text-xl justify-center gap-2 text-sm text-neutral-900 mb-1 animate-pulse">
    ...
    <span className="[font-family:'Dancing_Script',cursive] text-xl">
      Swipe to see more
    </span>
